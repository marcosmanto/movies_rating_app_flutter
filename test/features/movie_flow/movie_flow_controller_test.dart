...
void main() {
  ...
  group('MovieFlowControllerTests -', () {
    ...
    for (final rating in [0, 7, 10, 2, -2]) {
      test(
          'Given different ratings When updating rating with $rating Then that should be represented',
          () async {
        container
            .read(movieFlowControllerProvider.notifier)
            .updateRating(rating);
    ...
    for (final yearsBack in [0, 7, 70, 2, -2]) {
      test(
          'Given different yearsBack When updating rating with $yearsBack Then that should be represented',
          () async {
        container
            .read(movieFlowControllerProvider.notifier)
